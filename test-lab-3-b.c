...
void create1(const char *d, const char *f, const char *in) {
  int fd;
  char n[512];
...
void check1(const char *d, const char *f, const char *in) {
  int fd, cc;
...
void unlink1(const char *d, const char *f) {
  char n[512];
...
void checknot(const char *d, const char *f) {
  int fd;
  char n[512];
...
void append1(const char *d, const char *f, const char *in) {
  int fd;
  char n[512];
...
// one at a time.
void write1(const char *d, const char *f, int start, int n, char c) {
  int fd;
  char name[512];
  ...
  if (fd < 0 && errno == EEXIST) fd = open(name, O_WRONLY, 0666);
  if (fd < 0) {
    fprintf(stderr, "test-lab-3-b: open(%s): %s\n", name, strerror(errno));
    exit(1);
  }
...
void checkread(const char *d, const char *f, int start, int n, char c) {
  int fd;
  char name[512];
...
void createn(const char *d, const char *prefix, int nf, bool possible_dup) {
  int fd, i;
  char n[512];
  ...
  for (i = 0; i < nf; i++) {
    ...
    fd = creat(n, 0666);
    if (fd < 0 && possible_dup && errno == EEXIST) continue;
    if (fd < 0) {
      fprintf(stderr, "test-lab-3-b: create(%s): %s\n", n, strerror(errno));
      exit(1);
    }
...
void checkn(const char *d, const char *prefix, int nf) {
  int fd, i, cc, j;
  char n[512];
...
void unlinkn(const char *d, const char *prefix, int nf) {
  char n[512];
  int i;
...
int compar(const void *xa, const void *xb) {
  char *a = *(char **)xa;
  char *b = *(char **)xb;
  return strcmp(a, b);
}

void dircheck(const char *d, int nf) {
  DIR *dp;
  struct dirent *e;
  char *names[1000];
  int nnames = 0, i;
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < sizeof(huge) - 1; i++) huge[i] = '0';
