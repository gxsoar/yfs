...
class acceptor {
 private:
  ...
  std::map<unsigned, std::string>
      values;  // vals of each instance,
               // 只有在commit阶段才会碰到instance和value的存储
  ...
  paxos_protocol::status acceptreq(std::string src, paxos_protocol::acceptarg a,
                                   bool &r);
  paxos_protocol::status decidereq(std::string src, paxos_protocol::decidearg a,
                                   int &r);
...
class proposer {
 private:
  ...
  void breakpoint1();
  void breakpoint2();
  bool majority(const std::vector<std::string> &l1,
