...
int rpcc::call1(unsigned int proc, marshall &req, unmarshall &rep, TO to) {
  ...
  if (ca.done && lossytest_) {
    ...
    if (xid_rep > xid_rep_done_) xid_rep_done_ = xid_rep;
  }
...
bool rpcc::got_pdu(connection *c, char *b, int sz) {
  ...
  if (!ca->done) {
    ...
    if (ca->intret < 0) {
      jsl_log(JSL_DBG_2,
              "rpcc::got_pdu: RPC reply error for xid %d intret %d\n", h.xid,
              ca->intret);
    }
    ca->done = 1;
  }
...
compress:
  ...
  for (it++; it != xid_rep_window_.end(); it++) {
    while (xid_rep_window_.front() + 1 == *it) xid_rep_window_.pop_front();
  }
}
...
void rpcs::updatestat(unsigned int proc) {
  ...
  if (curr_counts_ == 0) {
    ...
    for (clt = reply_window_.begin(); clt != reply_window_.end(); clt++) {
      ...
      if (clt->second.size() > maxrep) maxrep = clt->second.size();
    }
    jsl_log(JSL_DBG_1,
...
void rpcs::dispatch(djob_t *j) {
  ...
  jsl_log(JSL_DBG_2,
          "rpcs::dispatch: rpc %u (proc %x, last_rep %u) from clt %u for srv "
          "instance %u \n",
  ...
  if (h.srv_nonce != 0 && h.srv_nonce != nonce_) {
    jsl_log(JSL_DBG_2,
            "rpcs::dispatch: rpc for an old server instance %u (current %u) "
            "proc %x\n",
  ...
  switch (stat) {
    case NEW:  // new request
      ...
      jsl_log(JSL_DBG_2,
              "rpcs::dispatch: sending and saving reply of size %d for rpc %u, "
              "proc %x ret %d, clt %u\n",
...
marshall &operator<<(marshall &m, bool x) {
  m.rawbyte(x);
  return m;
}
...
unmarshall &operator>>(unmarshall &u, bool &x) {
  x = (bool)u.rawbyte();
  return u;
}
...
int diff_timespec(const struct timespec &end, const struct timespec &start) {
  int diff =
      (end.tv_sec > start.tv_sec) ? (end.tv_sec - start.tv_sec) * 1000 : 0;
