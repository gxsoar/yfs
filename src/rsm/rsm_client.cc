...
rsm_protocol::status rsm_client::invoke(int proc, std::string req,
  ...
  while (1) {
    ...
    if (cl) {
      ret =
          cl->call(rsm_client_protocol::invoke, proc, req, rep, rpcc::to(5000));
    }
    ...
    if (ret == rsm_client_protocol::NOTPRIMARY) {
      printf(
          "primary %s isn't the primary--let's get a complete list of mems\n",
          primary.c_str());
      if (init_members()) continue;
    }
  prim_fail:
...
bool rsm_client::init_members() {
  ...
  VERIFY(pthread_mutex_lock(&rsm_client_mutex) == 0);
  if (cl == 0 || ret != rsm_protocol::OK) return false;
  if (new_view.size() < 1) {
