...
void testmarshall() {
  ...
  m.take_buf(&b, &sz);
  VERIFY(sz ==
         (int)(RPC_HEADER_SZ + sizeof(i) + sizeof(l) + s.size() + sizeof(int)));
...
void *client1(void *xx) {
  ...
  for (int i = 0; i < 100; i++) {
    ...
    if (ret != 0) printf("%d ms have elapsed!!!\n", diff);
    VERIFY(ret == 0);
...
void simple_tests(rpcc *c) {
  ...
  std::string rep;
  int intret =
      c->call(22, (std::string) "hello", (std::string) " goodbye", rep);
...
void failure_test() {
  ...
  printf(
      "   -- create new client and try to bind to failed server .. failed "
      "ok\n");
  ...
  std::string rep;
  int intret =
      client->call(22, (std::string) "hello", (std::string) " goodbye", rep);
  ...
  printf(
      "   -- delete existing rpc client, create replacement rpc client .. "
      "ok\n");
  ...
  VERIFY(client->bind() >= 0);
  printf(
      "   -- delete existing rpc client and server, create replacements.. "
      "ok\n");
